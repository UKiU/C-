...
void horizontal(vector<epos>* eline,double Enum, substrate* Sub, double subnum, deposit* &result)
{
	//材料最大宽度
	...
	for( int i=0;i < (int)widthMax;i++)
	{
		cout<<"redius="<<result[i].r<<endl;
		cout<<"dE/dS="<<result[i].deds<<" kev/nm"<<endl;
		cout<<endl;
	}
	delete[] result;
}